...
const { TOKEN, CLIENT_ID, ADMINS, ALLOWED_CATEGORY_ID } = require("./config");
const { categories: catObj, lastId: lastIdObj } = require("./categories");
const commandsArr = require("./commands");

let categories = catObj;
let lastId = lastIdObj;
...
(async () => {
  try {
    await rest.put(Routes.applicationCommands(CLIENT_ID), {
      body: commandsArr.map((cmd) => cmd.toJSON()),
    });
...
module.exports = { client, categories, lastId, ADMINS, ALLOWED_CATEGORY_ID };
